...
const getProducts = (req: Request, res: Response, next: NextFunction) => {
  res.render("./shop/product-list", {
    products:
      productData.dummyProductsArray.length > 0
        ? productData.dummyProductsArray
        : [],
    pageTitle: "Products",
    path: "/shop/products",
    hasProducts: productData.dummyProductsArray.length > 0,
...
const deleteProduct = (req: Request, res: Response, next: NextFunction) => {
  ...
  productData.dummyProductsArray = productData.dummyProductsArray.filter(
...
const addToCart = (req: Request, res: Response, next: NextFunction) => {
  ...
  productData.findById(productId, (product: Product) => {
...
const getProductDetails = (req: Request, res: Response, next: NextFunction) => {
  ...
  const product = productData.dummyProductsArray.find(
